...
PathFinder::Path PathFinder::findPath(Node * start, Node * end)
{
    ...
    while (!openNodes.empty())
    {
        ...
        Node * pNode = openNodes.back();
        openNodes.pop_back();
        if (pNode->isClosed())
        {
            // The node is already closed, so skip it. This can happen when an open node is updated with a lower cost (a duplicate
            // is created).
            continue;
        }
        pNode->close();
        ...
        for (auto const& edge : edges)
        {
            ...
            // If the neighbor is not in the open queue, then open it. Otherwise, update the open listing if the cost is cheaper..
            if (!pNeighbor->isOpen())
            ...
            // Add the new (or updated) neighbor to the open queue. If the neighbor was already in the open queue, then this will
            // create a duplicate entry and the higher cost will be skipped when it is popped.
